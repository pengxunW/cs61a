...
def boar_brawl(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 2
    # boar_brawl（82，115），shi_wei 取倒数第二位
    shi_wei = (opponent_score // 10) % 10
    ge_wei = player_score % 10
    score = 1
    suspect = 3 * abs(shi_wei - ge_wei)
    if score < suspect:
        score = suspect
    return score
    # END PROBLEM 2
...
def is_always_roll(strategy, goal=GOAL):
    ...
    False
    """
    base_roll_result = strategy(0, 0)
    # BEGIN PROBLEM 7
    for score1 in range(goal):
        for score2 in range(goal):
            roll_result = strategy(score1, score2)
            if base_roll_result != roll_result:
                return False
    return True
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    for num in range(1, 11):
        temp_score = averaged_score(num, dice)
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    deta_sus_score = sus_update(0, score, opponent_score) - score
    if deta_sus_score >= threshold:
        num_rolls = 0
    return num_rolls  # Remove this line once implemented.
    # END PROBLEM 11
